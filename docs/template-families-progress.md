...
### A. Paper（手账纸）⭐ 推荐优先
- **视觉**：米白/米黄纸张底，虚线/点线边框，钢笔风格手写感标题，邮票角/折叠角装饰
- **关键元素**：`dashed` / `dotted` border；纸张纹理（淡色 `lines` 背景）；标题用文楷字体
- **难度**：低（主要是 CSS 样式差异，结构与 classic 接近）
- **适合主题**：template-c（奶油琥珀）、template-f（陶土暖褐）

### B. Bento（便当格）⭐ 推荐优先
- **视觉**：不对称网格布局（Bento Box 风格）：Cover 是一个大格 + 右侧两个小格的 L 形；内容页用大小格混搭
- **关键元素**：CSS Grid 的 `grid-column: span 2` 混搭；各格填充不同背景色或图片
- **难度**：中（布局逻辑较复杂，需要按 slide type 定制网格分配）
- **适合主题**：全部主题
